...
const SingleRencardPost = (props) => {
    const [loading, setLoading] = React.useState(false);
    const [Rencard, setRencard] = React.useState({});
    React.useEffect(() => {
        setLoading(true);
        var formData = new FormData();
        formData.append('url', '/RESTAPI/Rencard/getSingle')
            ...
            <div className="ELG App">
                <div className="Fdo">
                    <div className="default-container App">
                        <ModalShowHeader title="Le rencontre que vous avez lancer" onClick={() => props.navigate(-1)} />
                        {!loading && Rencard.author &&
                            <div className="card-top" style={{ paddingTop: 0 }}>
                                <CardAuthorTemplate author={Rencard.author} />
                            </div>
                        }
                        ...
                        <div className="Fdo Flk Lsy LDv">
                            <div className="Fdo Anv Pac">
                                <div className="">choisir parmis les interessé</div>
                                {!loading &&
                                    <InterestedList post={Rencard} />
                                }
                            </div>
                        </div>
                    </div>
...
export default withRouter(SingleRencardPost)