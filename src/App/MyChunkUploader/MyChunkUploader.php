...
namespace  Src\App\MyChunkUploader;
...
class MyChunkUploader  {
	...
	function __construct( $working_dir = null , $app ,$getID3) {
	...
	private function _merge_files($dossier,$filename) {
		...
		foreach ( $files as $chunk_filename ) {
			$this->_copy_file( $chunk_filename,$dossier.$filename, 'final', 'ab' );
			unlink( $chunk_filename );
		}
	 ...
	 */
	public function run($dossier, $setfilename) {
		if ( ! $this->may_run() ) {
			return false;
		}
		...
		if ( ( $this->_range[2] + 1 == $this->_range[3] ) ) {
			$image_type = explode("/", $this->_get_content_type());
			if(isset($_POST['params'])){
				$params = json_decode($_POST['params'], true);
			}
			$newname = substr($_POST['media_appartenance'],0,1) .$_POST['albumId'].'-'.$this->app->session_id() .'_'.$setfilename .'.'. $image_type[1];
			$chemin = $dossier .DIRECTORY_SEPARATOR. $image_type[0].'s'. DIRECTORY_SEPARATOR;
			if ( $chunks = $this->_merge_files($chemin, $newname) ) {
				$get = $this->getID3->analyze($chemin. $newname);
				if($image_type[0]=== 'video' && htmlspecialchars($_POST['media_appartenance']) != 'message'){
					$thumbname = explode(".", $newname);
					$imageData = str_replace('data:image/png;base64,', '', $params['thumbnail']);
					$data = base64_decode($imageData);
					$file = $dossier .DIRECTORY_SEPARATOR.'thumbnail'.DIRECTORY_SEPARATOR.$thumbname[0].'.png';
					@file_put_contents($file, $data);
				}
				try {
					$this->app->Table('image')->create('medias',[
						'author_id' => $this->app->session_id(),
						'createdAt'=> date('Y-m-d H:i:s'),
						'media_name'=>$newname,
						'element_id' => $_POST['albumId'],
						'media_type'=> $image_type[0],
						'createdAt' => date('Y-m-d H:i:s'),
						'montant' => isset($_POST['params']) ? (int)$params['montant'] : 0,
						'media_height'=> $get['video']['resolution_y'],
						'media_width'=>$get['video']['resolution_x'],
						'description' => isset($_POST['params']) ? $this->app->trim(htmlspecialchars($params['texte'])) : null,
						'media_apartenance'=> htmlspecialchars($_POST['media_appartenance'])
					]);
					...
					unlink($dossier .DIRECTORY_SEPARATOR. $newname);
				}
			} else
