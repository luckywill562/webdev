...
function http_stream_file($file_path) {
  ...
  if((int)$confident->montant > 0 && !$payment && $confident->user_id != $session_user_id){
    die('vous n\'avez pas le droit de voir cette video');
 }else{
   http_stream_file($file_path .'videos/'.$file );
 }