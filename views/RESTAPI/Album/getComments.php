...
}elseif($app->IsAjax() && $auth->isAuth() && isset($_POST['album_id']) && 
!empty($_POST['album_id']) && isset($_POST['type']) && !empty($_POST['type']) 
&& isset($_POST['limit']) && !empty($_POST['limit'])
&& isset($_POST['page'])){
    try {
        $comment_Data=[];
        foreach($commentTable->FetchComment('album_comments','element_id',$_POST['album_id'], $_POST['type'],$_POST['limit'], $_POST['page'] ) as $comments => $comment){
